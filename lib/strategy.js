...
/**
 * Retrieve user profile from Keylead.
 *
 * This function constructs a normalized profile, with the following properties:
 *
 *   - `provider`         always set to `keylead`
	...
	this._oauth2.get(url, accessToken, function (err, body, res) {
		...
		if (err) {
			...
			if (json && json.error && typeof json.error == 'object') {
				return done(new KeyleadAPIError(json.error.message, json.error.type, json.error.code, json.error.error_subcode));
			}
